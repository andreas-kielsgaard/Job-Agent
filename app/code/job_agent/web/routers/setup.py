...
from job_agent.llm import DEFAULT_CLAUDE_MODEL, ExternalAgentService, LlmRequest
...
def setup(request: Request) -> HTMLResponse:
    return templates.TemplateResponse(
        request, "setup.html", {"request": request, **workflow_handler().profile.setup_view()}
    )
...
@router.post("/setup/cv-reference/external-agent/prepare")
async def prepare_external_profile_draft(request: Request) -> JSONResponse:
    form = await request.form()
    profile = workflow_handler().profile
    targets = profile.auto_config_targets_from_form(form)
    ...
    try:
        ...
        interaction = ExternalAgentService(profile.root).prepare(
            ...
            instructions=(
                "Paste this prompt into an external agent. Paste back only the JSON object so the app can "
                "preview and apply the selected profile sections."
            ),
            metadata={"targets": targets, "source_label": "current reference CV"},
        )
...
@router.post("/setup/cv-reference/external-agent/apply")
async def apply_external_profile_draft(request: Request) -> JSONResponse:
    form = await request.form()
    profile = workflow_handler().profile
    interaction_id = str(form.get("interaction_id") or "")
    response_text = str(form.get("response_text") or "")
    try:
        interaction = ExternalAgentService(profile.root).load(interaction_id)
        completion = ExternalAgentService(profile.root).complete(interaction_id, response_text)
        targets = [str(item) for item in interaction.metadata.get("targets", [])]
        draft = profile.draft_profile_auto_configuration_from_response(completion.text, targets)
        draft_record = profile.save_draft(
            draft,
            source_label=str(interaction.metadata.get("source_label") or "external agent"),
            task_id="",
        )
    ...
    return JSONResponse(
        {
            "ok": True,
            "redirect_url": f"/setup?{urlencode({'message': 'Profile draft preview is ready for review.'})}#cv-profile-draft",
