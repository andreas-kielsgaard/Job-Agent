    ...
    *,
    ...
    if preview:
        state["preview"] = {
            ...
            "run_steps": [
                {"phase": step.phase, "status": step.status, "detail": step.detail} for step in preview.run_steps
            ],
            "field_coverage": [
            ...
            "jobs": [{"title": job.title, "url": job.url, "location": job.location} for job in preview.jobs[:10]],
        }
    if source_review:
        state["source_review"] = {
...
def _source_review_from_payload(payload: dict, selected_source) -> dict:
    ...
    return {
        ...
        "sample_titles": [
            str(job.get("title") or "") for job in (payload.get("jobs") or [])[:5] if isinstance(job, dict)
        ],
        "sample_urls": [
            str(job.get("url") or "")
            for job in (payload.get("jobs") or [])[:5]
