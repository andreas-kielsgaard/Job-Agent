...
class RecipeWorkflowHandler:
    ...
    def capture_calibration(
        ...
        *,
        ...
        session_state_path = (
            resolve_project_path(self.root, session_status.storage_state_path) if session_status.usable else None
        )
...
class ProfileWorkflowHandler:
    ...
    def profile_auto_configuration_prompt(self, cv_text: str, targets: list[str], *, progress_callback=None) -> str:
        return self.setup.profile_auto_configuration_prompt(
            cv_text,
            targets,
            progress_callback=progress_callback,
        )

    def draft_profile_auto_configuration_from_response(self, response_text: str, targets: list[str]):
        return self.setup.draft_profile_auto_configuration_from_response(response_text, targets)
