...
def build_profile_contract(root: Path = ROOT, cv_reference: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    contact_yaml = _dict(read_yaml(profile / "contact.yaml", {}))
    contact = _dict(contact_yaml.get("contact", {}))
    preferences = _dict(read_yaml(profile / "preferences.yaml", {}))
    skills_yaml = _dict(read_yaml(profile / "skills.yaml", {}))
    experience_yaml = _dict(read_yaml(profile / "experience.yaml", {}))
...
def _diagnostics(
    *,
    ...
    if not contact.get("name") or not contact.get("email"):
        diagnostics.append(
            _diagnostic(
                "warning", "Identity needs basics", "Name and email are needed for generated CVs and form answers."
            )
        )
    ...
    if cv_reference.get("extraction_error"):
        diagnostics.append(_diagnostic("error", "CV text extraction failed", cv_reference["extraction_error"]))
    ...
    if not writing_style.strip():
        diagnostics.append(
            _diagnostic(
                "info", "Writing style is empty", "AI-assisted writing will still work, but with less style guidance."
            )
        )
    if not diagnostics:
        diagnostics.append(
            _diagnostic("ok", "Profile contract looks usable", "No immediate profile consistency issues found.")
        )
    return diagnostics
