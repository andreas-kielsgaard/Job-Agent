...
from job_agent.paths import recipes_dir, resolve_project_path, sources_dir
...
class SourceRegistryService:
    def __init__(self, root: Path = ROOT) -> None:
        self.root = root
        self.path = sources_dir(root) / "source-registry.yaml"
    ...
    def _infer_recipe_state(self, recipe_path: str, tags: list[str], status: str) -> str:
        ...
        if "partial" in lowered_tags or status == "needs_review":
            return "partial"
        path = resolve_project_path(self.root, recipe_path)
...
def _stats_from_packages(packages: list[dict[str, Any]]) -> SourceStats:
    ...
    sorted_packages = sorted(
        packages, key=lambda item: (item.get("run_id", ""), item.get("match_score", 0)), reverse=True
    )
    ...
    weak_or_excluded_matches = sum(1 for package in packages if package.get("match_category") in {"weak", "excluded"})
...
def _value_summary(
    ...
    *,
    ...
    if value_status == "promising":
        return (
            f"{jobs_found_total} saved jobs with {strong_matches} strong and {exploratory_matches} exploratory matches."
        )
...
def _validate_recipe_path(value: str, root: Path) -> str:
    ...
    if not recipe_path:
        return ""
    candidate = resolve_project_path(root, recipe_path)
    try:
        resolved = candidate.resolve()
        recipes_root = recipes_dir(root).resolve()
    ...
    try:
        return f"sources/recipes/{resolved.relative_to(recipes_root).as_posix()}"
    except ValueError:
