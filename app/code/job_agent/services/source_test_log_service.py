...
class SourceTestMaterialLog:
    ...
    def relative_dir(self) -> str:
        return display_path(self.root, self.artifact_dir)
    ...
    def relative_manifest_path(self) -> str:
        return display_path(self.root, self.manifest_path)
    ...
    def record_text(self, filename: str, content: str, *, kind: str, metadata: dict[str, Any] | None = None) -> str:
        ...
        entry = {"kind": kind, "path": display_path(self.root, path), "bytes": len(content.encode("utf-8"))}
        if metadata:
    ...
    def record_json(self, filename: str, payload: Any, *, kind: str, metadata: dict[str, Any] | None = None) -> str:
        ...
        entry = {"kind": kind, "path": display_path(self.root, path)}
        if metadata:
            entry.update(_jsonable(_redact_mapping(metadata)))
        self.entries.append(entry)
        return entry["path"]
    ...
    def record_html(
        ...
        *,
        ...
        entry = {
            ...
            "final_url": final_url,
            "mode": mode,
            "html_path": display_path(self.root, html_path),
            "text_path": display_path(self.root, text_path),
    ...
    def _new_artifact_dir(self) -> Path:
        ...
        base = output_dir(self.root) / "source-tests" / self.source_id / timestamp
        path = base
        counter = 1
        while path.exists():
            counter += 1
