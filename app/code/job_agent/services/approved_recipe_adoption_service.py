...
from job_agent.paths import display_path, recipes_dir, resolve_project_path
...
class ApprovedRecipeAdoptionService:
    ...
    def _validate_approved_recipe_path(self, value: str) -> str:
        path = Path(value)
        resolved = path.resolve() if path.is_absolute() else resolve_project_path(self.root, path).resolve()
        ...
        return f"sources/recipes/{resolved.relative_to(base).as_posix()}"
