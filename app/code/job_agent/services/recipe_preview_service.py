...
def explain_recipe(recipe_path: str | Path, root: Path | None = None) -> RecipeExplanation | None:
    ...
    try:
        ...
        return None
    listing_fields = (
        _api_listing_explanation_fields(recipe)
    ...
    pagination_configured = (recipe.listing_api.url and recipe.listing_api.pagination.strategy != "none") or bool(
    ...
    navigation_fields = [
        ...
        RecipeExplanationItem(
            "Detail structured data", "Uses JobPosting JSON-LD." if recipe.detail.use_json_ld else "Not configured."
        ),
        RecipeExplanationItem(
            "Detail API",
...
def preview_recipe(
    ...
    resolved_recipe_path = _resolve_path(recipe_path, root)
    recipe = load_job_board_recipe(resolved_recipe_path)
    extraction = _run_preview_extraction(
        recipe, input_value, base_url=base_url, rendered=rendered, static=static, root=root
    )
    ...
    if detail_value:
        ...
        if not recipe.detail.follow:
            warnings.append(
                "Recipe parsed the detail sample, but detail.follow is false; URL runs will not fetch job detail pages."
            )
...
def _field_coverage(jobs: list[Job]) -> list[FieldCoverage]:
    return [
        FieldCoverage(
            field=field, present_count=sum(1 for job in jobs if _field_present(job, field)), total_count=len(jobs)
        )
...
def _request_notes(recipe, input_type: str, has_detail_sample: bool) -> list[str]:
    ...
    if input_type == "public URL":
        notes.append(
            "Preview is a proof run: it fetches the configured listing data and only enough follow-up requests to prove configured navigation."
        )
        if recipe.detail.follow:
            notes.append(
...
def _run_steps(
    ...
    steps.extend(PreviewRunStep(phase=step.phase, status=step.status, detail=step.detail) for step in extraction.steps)
    steps.extend(
        [
            PreviewRunStep(
            ...
            PreviewRunStep(
                ...
                status="completed"
                if not any(check.status == "fail" for check in extraction.capability_checks)
                else "warning",
