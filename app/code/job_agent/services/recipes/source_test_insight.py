...
def apply_source_test_insight_to_recipe(recipe_data: dict, prompt_payload: dict[str, Any]) -> list[str]:
    ...
    if required_strategy == "browser_click" and current_strategy != "browser_click":
        ...
        if click_selector:
            ...
            return warnings
        warnings.append("Source test requires browser-click pagination, but no click selector evidence was available.")
        return warnings
    ...
    ajax_template = _best_ajax_pagination_template(
        prompt_payload.get("observed_ajax_pagination_templates"), recipe_data
    )
    request_delay = pagination.get("request_delay_seconds", 1.0)
    max_pages = _max_pages_from_pagination_evidence(pagination, prompt_payload)
    if ajax_template:
...
def _best_browser_click_selector(prompt_payload: dict[str, Any], pagination: dict[str, Any]) -> str:
    controls = (
        prompt_payload.get("observed_interactive_pagination_controls") if isinstance(prompt_payload, dict) else []
    )
...
def _max_pages_from_pagination_evidence(pagination: dict[str, Any], prompt_payload: dict[str, Any]) -> int:
    ...
    for item in (
        prompt_payload.get("observed_ajax_pagination_templates", []) if isinstance(prompt_payload, dict) else []
    ):
        if isinstance(item, dict):
