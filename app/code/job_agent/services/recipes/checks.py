...
def capability_checks(result: RecipeExtractionResult, recipe: JobBoardRecipe) -> list[RecipeCapabilityCheck]:
    ...
    pagination_duplicate_blocked = (
        pagination_is_expected and result.pagination_fetch_count > 0 and pagination_blocking_duplicate_listing_count > 0
    )
    pagination_strategy_failed = bool(
        ...
        or (
            ...
            and recipe.listing_api.pagination.max_pages > 1
            and not result.pagination_fetch_count
        )
    )
    ...
    return [
        ...
        RecipeCapabilityCheck(
            ...
            status="pass"
            if api_listing_expected and result.jobs
            else "fail"
            if api_listing_expected
            else "not_expected",
        ...
        RecipeCapabilityCheck(
            ...
            observed=bool(
                api_pagination_expected
                or result.pagination_fetch_count
                or result.pagination_links
                or interactive_controls_observed
            ),
