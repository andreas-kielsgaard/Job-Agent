...
from job_agent.paths import recipes_dir, resolve_project_path
...
SELECTOR_FIELDS = [
    ...
    (
        "detail.workload_selector",
        "Detail workload",
        "Contract type, full-time, or workload text on the detail page.",
        False,
    ),
...
class RecipeEditorService:
    ...
    def save_selectors(self, recipe_path: str, values: dict[str, str]) -> None:
        ...
        for field_path, _label, _help_text, required in SELECTOR_FIELDS:
            ...
            _nested_set(
                data,
                field_path,
                _selector_value(value, single=field_path == "listing.card_selector", required=required),
            )
    ...
    def resolve_recipe_path(self, value: str) -> Path:
        ...
        if not candidate.is_absolute():
            candidate = resolve_project_path(self.root, candidate)
        try:
            resolved = candidate.resolve()
            recipes_root = recipes_dir(self.root).resolve()
    ...
    def _recipe_paths(self) -> list[str]:
        recipes_root = recipes_dir(self.root)
