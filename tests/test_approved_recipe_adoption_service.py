...
def test_prepare_disabled_execution_entry_refuses_enabled_daily_run_entry(project_root: Path) -> None:
    candidate = _approved_candidate(project_root)
    config_path = project_root / "sources" / "recruiting-sites.yaml"
    config_path.write_text(
        "sources:\n"
    ...
    with pytest.raises(ValueError, match="enabled for daily runs"):
    ...
    entry = read_yaml(config_path, {})["sources"][0]
    assert entry["recipe_path"] == "old.yaml"
