...
class SetupGuideService:
    ...
    def build_context(self, *, current_path: str = "") -> dict[str, Any]:
        ...
        if active_step:
            ...
            active_step["companion_title"] = (
                "Welcome. I'll guide you through setup."
                if active_step["id"] == steps[0]["id"]
                else "Next setup step"
            )
    ...
    def _observed_state(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "cv": {
                "complete": bool(cv_reference or _read_text(self.root / "profile" / "canonical-cv.md").strip()),
                "detail": (
    ...
    def _profile_observed(self, profile_contract: dict[str, Any]) -> dict[str, Any]:
        ...
        preferences = read_yaml(self.root / "profile" / "preferences.yaml", {})
    ...
    def _source_observed(self) -> dict[str, Any]:
        config = read_yaml(self.root / "sources" / "recruiting-sites.yaml", {"sources": []})
        ...
        registry = read_yaml(self.root / "sources" / "source-registry.yaml", {"sources": []})
        ...
        readiness = read_yaml(self.root / "sources" / "source-execution-readiness.yaml", {"sources": {}})
    ...
    def _run_observed(self) -> dict[str, Any]:
        runs = read_json(self.root / "output" / "runs" / "runs.json", [])
        ...
        active_or_done = [
            ...
            if str(run.get("status") or "") in {"pending", "running", "completed"}
        ]
        if active_or_done:
