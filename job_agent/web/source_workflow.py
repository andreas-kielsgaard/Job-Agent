...
class SourceWorkflowHandler:
    ...
    def build(self, source_id: str) -> SourceWorkflowState | None:
        ...
        setup_complete = all(
            str(step.get("state") or "") == "complete"
    ...
    def source_session_status(self, source):
        ...
        if source.recipe_path:
            try:
                recipe = load_job_board_recipe(self.root / source.recipe_path)
    ...
    def source_test_insight(self, source, *, result=None, readiness=None) -> dict[str, Any]:
        ...
        if result is not None:
            ...
            pagination_duplicate_page_count = int(
                getattr(readiness, "dry_run_pagination_duplicate_page_count", 0) or 0
            )
        ...
        pagination_warning = None if pagination_failure else pagination_warning_signal(
            warnings=warnings,
        ...
        if recipe_changed_after_source_test:
            ...
            recommendation = "Run a fresh safe source test so the app verifies the updated plan before indexing or daily runs."
            ...
            recommendation = "Run the safe source test again; if this repeats, review the source session and page access."
            action = {"type": "link", "label": "Run source test", "href": f"/sources/{source.id}/test-run?start=1"}
