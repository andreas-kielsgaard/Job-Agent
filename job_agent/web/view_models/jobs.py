...
def _filter_jobs(jobs: list[dict[str, Any]], filters: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for job in jobs:
        ...
        if filters.get("ai_prioritized") and not _truthy(job.get("ai_should_prioritize")):
            continue
        if source_text and source_text not in " ".join(
...
def _with_run_context(
    service: PackageIndexService, job: dict[str, Any], run_labels: dict[str, str]
) -> dict[str, Any]:
    enriched = dict(job)
    ...
    enriched["source_url"] = str(enriched.get("source_url") or enriched.get("url") or enriched.get("application_url") or "")
    return enriched
...
def _source_options(root: Path, filtered_jobs: list[dict[str, Any]], all_jobs: list[dict[str, Any]]) -> list[dict[str, str]]:
...
def _run_options(root: Path, filtered_jobs: list[dict[str, Any]], all_jobs: list[dict[str, Any]]) -> list[dict[str, str]]:
