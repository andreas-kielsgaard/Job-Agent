...
def save_env(
    ...
    setup_service().save_env_settings(anthropic_api_key, claude_model, claude_use_by_default)
...
def save_preferences(
    ...
    setup_service().save_preferences(
...
async def upload_cv_reference(
    cv_file: UploadFile = File(...), extract_to_canonical: bool = Form(False)
) -> RedirectResponse:
    try:
...
async def _store_uploaded_cv(cv_file: UploadFile, extract_to_canonical: bool) -> None:
    if not cv_file.filename:
        return
    cv_reference_service().store_reference_cv(cv_file.filename, await cv_file.read(), extract_to_canonical)
...
def save_setup_file(file_key: str = Form(...), content: str = Form(...)) -> RedirectResponse:
    try:
        setup_service().save_setup_file(file_key, content)
